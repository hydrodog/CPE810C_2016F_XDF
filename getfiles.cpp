...
#include <QString>
using namespace std;
string nameoffiles[10000];//save names of all the files
string pathoffiles[10000];//save the path of all the files
string pathofimage[10000];//save the path of all the images(jpg,png,flif)
QString imagepath[10000];//for image team，tranform into QString
int countnumber;
/************************************************************************/
/*  get all name and path
    import：
        path	:	path of folder
...
void getFiles( string path, string exd, vector<string>& files )
{
    ...
    if((hFile = _findfirst(pathName.assign(path).append(exdName).c_str(),&fileinfo)) !=  -1)
    {
        do
        {
            ...
            //if not, get it
            if((fileinfo.attrib & _A_SUBDIR))//is it a folder?
            {
...
int main()
{
    ...
    vector<string> files;//define a string type files
    getFiles(filePath, "", files);//get all the files in the path
    int size = files.size();//numbers of files
    for (int i = 0;i < size;i++)
    {
        pathoffiles[i] = files[i].c_str();//sve the path
